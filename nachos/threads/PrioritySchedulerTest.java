...
public class PrioritySchedulerTest {
          ...
          while(amIDone == false) {
            ...
            for (int i=locks.length-1; i >= 0; i--) {
              System.out.println(this.name+" about to release "+
                                 locks[i].getName() + " P: " + KThread.currentThread().getPriority()
                                 + " EP: " + KThread.currentThread().getEffectivePriority());
              locks[i].release();
    ...
    private static void runPriorityDonationTest3() {
        ...
        for (int i=0; i < testSize; i++) {
          int priority = (int) (5+((Math.random())*(7-5)+1));
